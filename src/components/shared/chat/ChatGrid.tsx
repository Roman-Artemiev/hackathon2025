  ...
  return (
    <div className="grid-cols-2 grid gap-1">
      {gridItems?.map((item, i) => (
        <div
          key={i}
          className={`max-w-[75%] py-2 px-3 rounded-xl text-sm leading-relaxed ${
            item.author === 0
...
export function ThinkingChatGrid() {
  return (
    <div className="grid-cols-2 grid gap-1">
      <div
        className={`max-w-[75%] py-2 px-3 rounded-xl text-sm leading-relaxed col-span-2 justify-self-start bg-gray-200 text-gray-800 rounded-bl-none font-black`}
      >
        <p className="animate-pulse text-3xl m-0">...</p>
      </div>
    </div>
  );
}
